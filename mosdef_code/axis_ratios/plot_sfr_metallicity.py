...
def plot_sfr_metals(save_name, plot_ssfr=False, plot_re=False, plot_sanders=False):
    ...
    def solve_re_func(A_lambda, sfr_data, metal_data):
        def re_func(sfr, re):
            metallicity = np.log10(A_lambda / ((const2) * ((sfr/(re**2))**(1/n)))) / a
            return metallicity

        breakpoint()
        popt, pcov = curve_fit(re_func, sfr_data, metal_data)
        re_val = popt[0]
        return re_val
    ...
    A_lambda = 0.8
    # re = 0.25
    lowmass_gals = summary_df['log_mass_median']<10
    # re = solve_re_func(A_lambda, summary_df[lowmass_gals]['log_use_sfr_median'], summary_df[lowmass_gals]['metallicity_median'])
    # print(f're_lowmass = {re}')
    re = 1.03
    ...
    slope_lowmass, yint_lowmass = get_slope(x_plot[0], x_plot[-1], metal_vals[0], metal_vals[-1])
    
    def point_dist(x1, y1, m, yint):
        Distance = (np.abs(-m*x1 + y1 - yint)) / np.abs(m)
        return Distance
    dist = point_dist(summary_df[lowmass_gals]['log_use_sfr_median'], summary_df[lowmass_gals]['metallicity_median'], slope_lowmass, yint_lowmass)
    print(f'low mass: {np.sum(dist**2)}')
    ...
    slope_highmass, yint_highmass = get_slope(x_plot[0], x_plot[-1], metal_vals[0], metal_vals[-1])
    dist = point_dist(summary_df[~lowmass_gals]['log_use_sfr_median'], summary_df[~lowmass_gals]['metallicity_median'], slope_highmass, yint_highmass)
    print(f'high mass: {np.sum(dist**2)}')
    ...
    # ax.text(0.18, 8.575, 'A$_\mathrm{balmer} = 0.85$', fontsize=16, rotation=308, color='#8E248C')
    ax.text(0.375, 8.4925, 'A$_\mathrm{balmer} = 0.85$', fontsize=16, rotation=308, color='#8E248C')
    ax.text(0.80, 8.76, 'A$_\mathrm{balmer} = 1.9$', fontsize=16, rotation=308, color='#FF640A')
