...
def setup_transfer_script(mosdef_df=mosdef_df):
    ...
    image_f = open(
        imd.home_dir + '/mosdef/HST_Images/transfer_images.sh', 'w')
    spec_f = open(
        imd.home_dir + '/mosdef/HST_Images/transfer_spectra.sh', 'w')
...
def read_cat(field):
    ...
    # Read the data
    cat_location = imd.home_dir + '/mosdef/Catalogs/' + cat_dict[field]
...
def get_sed(field, v4id, full_cats_dict=False):
    ...
    # Here we split to create the obj - different based on if we need to match
    # ra/dec or just look it up
    if field == 'GOODS-S' or field == 'UDS' or field == 'COSMOS':
        ...
        # if len(mosdef_obj) > 1:
        #     f = open(
        #         imd.home_dir + '/mosdef/sed_csvs/sed_errors/duplicate_objs.txt', 'a')
        ...
        if d2d_match > 8.33 * 10**-5 * u.deg:
            ...
            f = open(
                imd.home_dir + '/mosdef/sed_csvs/sed_errors/zfourge_no_match.txt', 'a')
            f.write(f'{field}, {v4id}, {d2d_match}\n')
            f.close()
            # Add the _3DHST string so that it will use 3DHST when reading the
            # catalog
            str_3dhst = '_3DHST'
            # Failed match, so we will need to re-read the cat later with the
            # 3D_HST tag
            matched = False
        else:
            # If the match was successful, set matched to true since we have
            # our obj and have read the cat
            obj = cat.iloc[int(idx_match)]
            matched = True
    else:
        # If not in ZFOURGE, match is false so it will read the obj and cat
        matched = False
    # If it is unmatched at this point, read the correct catalog and get the
    # obj
    if matched == False:
        if full_cats_dict:
            cat = full_cats_dict[field + str_3dhst]
    ...
    filters_df = ascii.read(imd.home_dir + '/code/mosdef_code/filters/catalog_filters/' + field + str_3dhst +
                            '_filterlist.csv').to_pandas()
    if v4id == -9999:
        print(f'Error: ID of -9999')
        f = open(
            imd.home_dir + '/mosdef/sed_csvs/sed_errors/other_errors.txt', 'a')
        f.write(f'{field}, {v4id}\n')
        f.close()
        return None
    ...
    # Merge with redshift, field, and v4id, as these will be useful to have in
    # each line for constructing composite seds
    ...
    sed.to_csv(imd.home_dir + f'/mosdef/sed_csvs/{field}_{v4id}{str_3dhst}_sed.csv', index=False)
    return sed
