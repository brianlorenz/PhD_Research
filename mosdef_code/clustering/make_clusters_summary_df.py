...
def make_clusters_summary_df(n_clusters, ignore_groups, use_ha_first_csvs=False, halpha_scaled=False):
    ...
    for groupID in range(n_clusters):
        ...
        lum_weights = flux_to_luminosity_factor(group_df['Z_MOSFIRE'])
        lum_weighted_mass = weighted_quantiles((10**group_df['log_mass']).to_numpy(), lum_weights)
        median_lum_weighted_mass = np.log10(lum_weighted_mass)
        ...
        median_balmer = np.median(group_df[ok_balmer_rows]['balmer_dec'])
        ...
        median_lum_weighted_masses.append(median_lum_weighted_mass)
        ...
        median_balmers.append(median_balmer)
