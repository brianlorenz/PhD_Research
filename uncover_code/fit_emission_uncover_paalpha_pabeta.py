...
from compute_av import compute_paalpha_pabeta_av, get_nii_correction, compute_ha_paalpha_av
from plot_vals import scale_aspect
from simple_sample_selection import find_good_spec

emission_fit_dir = '/Users/brianlorenz/uncover/Data/emission_fitting_paalpha_pabeta/'
...
def fit_emission_uncover(spectrum, save_name, bootstrap_num=-1):
    ...
    ha_pab_av = [compute_paalpha_pabeta_av(1/ha_pab_ratio[i]) for i in range(len(line_list))]
    ...
    if n_loops > 0:
        ...
        all_avs = [compute_paalpha_pabeta_av(1/all_ha_pab_ratios[i]) for i in range(len(arr_popt))]
        ...
        monte_carlo_df = pd.DataFrame(zip(velocity_monte_carlo, all_ha_fluxes, all_pab_fluxes, all_ha_pab_ratios), columns = ['velocity', 'ha_flux', 'pab_flux', 'pab_paa_ratio'])
        imd.check_and_make_dir(emission_fit_dir)
    ...
    fit_df = pd.DataFrame(zip(line_names, line_centers_rest,
                              ...
                              columns=['line_name', 'line_center_rest', 'z_offset', 'err_z_offset', 
                                       ...
                                       'amplitude', 'err_amplitude', 'sigma', 'err_sigma', 'flux', 'err_flux', 'err_flux_low', 'err_flux_high', 'pab_paa_ratio', 'err_pab_paa_ratio_low', 'err_pab_paa_ratio_high', 'pab_paa_av', 'err_pab_paa_av_low', 'err_pab_paa_av_high', 'equivalent_width_aa'])
...
def plot_emission_fit(emission_fit_dir, save_name, total_spec_df, ax_plot='', plot_type=''):
    ...
    ax_Ha.text(0.05, 0.93, f"PaA: {round(10**17*fit_df.iloc[0]['flux'], 4)}", transform=ax_Ha.transAxes)
    ax_Hb.text(0.05, 0.93, f"PaBeta: {round(10**17*fit_df.iloc[1]['flux'], 4)}", transform=ax_Hb.transAxes)
    ax_Ha.text(0.05, 0.83, f"Ratio: {round(fit_df.iloc[0]['pab_paa_ratio'], 4)}", transform=ax_Ha.transAxes)
    ax_Hb.text(0.05, 0.83, f"Ratio: {round(fit_df.iloc[1]['pab_paa_ratio'], 4)}", transform=ax_Hb.transAxes)
...
def multi_gaussian(wavelength, *pars, fit=True):
    ...
    wavelength_ha = wavelength[wavelength < 15000]
    wavelength_pab = wavelength[wavelength > 15000]
...
def fit_all_emission_uncover(id_msa_list):
    for id_msa in id_msa_list:
        ...
        if np.min(spec_df.rest_wave_aa) > 11800:
            ...
            continue
        if np.max(spec_df.rest_wave_aa) < 20000:
