...
def compare_sed_flux(id_msa, make_plot=True):
    ...
    if make_plot == True:
        ...
        ax.plot(spec_df['wave'], spec_df['flux'], color='blue', marker='None', ls='--', label='SED')
        ax.plot(wave_micron, sed_jy, color='orange', marker='o', ls='None', label='Integrated Spectrum')
        ax.plot(wave_micron, sed_df['flux'], color='black', marker='o', ls='None', label='Spectrum')
        fontsize = 14
...
def compare_all_sed_flux(id_msa_list):
    ...
    for id_msa in id_msa_list:
        full_ratio, blue_ratio, red_ratio = compare_sed_flux(id_msa, make_plot=False)
