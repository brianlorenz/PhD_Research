...
def calc_lineflux(id_msa, fluxcal_str=''):
    ...
    pab_filters, pab_images, wht_pab_images, obj_segmap, pab_photfnus, pab_all_lines = make_3color(id_msa, line_index=1, plot=False, paalpha=True)
    ...
    pab_sedpy_transmission = get_transmission_at_line(pab_sedpy_filt, line_list[2][1] * (1+redshift), trasm_type='raw')

    pab_filters = ['f_'+filt for filt in pab_filters]

    pab_line_scaled_transmission = get_transmission_at_line(pab_sedpy_filt, line_list[2][1] * (1+redshift))

    pab_flux_erg_s_cm2, pab_cont_flux_erg_s_cm2_aa = measure_lineflux(id_msa, sed_df, redshift, pab_filters, pab_line_scaled_transmission, pab_sedpy_transmission, line_list[0][1], pab_sedpy_width, pab_sedpy_wave, pab_sedpy_filts)
    
    

    # Phot equivalent widths
    pab_phot_eq_width = pab_flux_erg_s_cm2 / pab_cont_flux_erg_s_cm2_aa

    # Integrated spectra
    spec_df = read_fluxcal_spec(id_msa)
    
    # For info about fluxcal factors
    pab_wave = np.logical_and(spec_df['rest_wave_aa'] > 6400, spec_df['rest_wave_aa'] < 6700) 
    ...
    int_spec_pab_flux_erg_s_cm2, _ = measure_lineflux(id_msa, int_spec_df, redshift, pab_filters, pab_line_scaled_transmission, pab_sedpy_transmission, line_list[0][1], pab_sedpy_width, pab_sedpy_wave, pab_sedpy_filts)
    ...
    fit_df = ascii.read(f'/Users/brianlorenz/uncover/Data/emission_fitting_pabeta_only/{id_msa}_emission_fits.csv').to_pandas()
    pab_flux_emission_fit = fit_df['flux'].iloc[0]
   

    pab_offset_factor = pab_flux_erg_s_cm2 / pab_flux_emission_fit

    return pab_flux_erg_s_cm2, pab_flux_cat_erg_s_cm2, err_pab_flux_cat_erg_s_cm2, pab_flux_emission_fit, int_spec_pab_flux_erg_s_cm2, pab_phot_eq_width
...
def calc_all_lineflux(id_msa_list, full_sample=False, fluxcal=True):
    ...
    for id_msa in id_msa_list:
        pab_flux_erg_s_cm2, pab_flux_cat_erg_s_cm2, err_pab_flux_cat_erg_s_cm2, pab_flux_emission_fit, int_spec_pab_flux_erg_s_cm2, pab_phot_eq_width = calc_lineflux(id_msa, fluxcal_str=fluxcal_str)
        ...
        int_spec_pab_nocors.append(int_spec_pab_flux_erg_s_cm2)
        pab_phot_eq_widths.append(pab_phot_eq_width)
    emission_offset_df = pd.DataFrame(zip(id_msa_list, pab_sed_fluxes, pab_cat_fluxes, err_pab_cat_fluxes, pab_emission_fits, int_spec_pab_nocors, pab_phot_eq_widths), columns=['id_msa', 'pab_sed_flux', 'pab_cat_flux', 'err_pab_cat_flux', 'pab_emfit_flux',  'int_spec_pab_nocor', 'pab_phot_eq_width'])
    ...
    print(f'\n\n\n')
    print(f'median offset in Ha {np.median(emission_offset_df["pab_sed_div_emfit"])}')
...
if __name__ == "__main__":
    ...
    pab_sample = [15350, 17089, 18045, 19283, 25774, 27621, 29398, 33157, 38987, 42203, 42238, 43497, 48463]
    supercat_df = read_supercat()
    id_dr3s = []
    for id_msa in pab_sample:
        id_dr3 = supercat_df[supercat_df['id_msa'] == id_msa]['id'].iloc[0]
        id_dr3s.append(id_dr3)
    breakpoint()
    calc_all_lineflux(pab_sample, fluxcal=True)  
